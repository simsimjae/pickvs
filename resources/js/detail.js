...
$(function() {
	...
	function Complain() {
		...
		oAjax
			.sendRequest(URL_CREATE_COMPLAIN, { writing_no }, null, 'POST')
			.then(isSuccess => {
	...
	ssj.view.comment.optionBar.prototype = $.extend(
		Object.create(ssj.view.common.comment),
		{
			init() {
				this._initVar();
				this._assignElements();
	...
	ssj.util.swiper.prototype = $.extend(
		{
			...
			refreshHeight() {
				...
				const slideHeight = $(this.getActiveSlide())
					.find('section')
					.innerHeight();
	...
	ssj.view.mention.prototype = $.extend(
		{
			...
			_clearMention() {
				this.$mentionNickname.text('');
				this.$mentionCommentNo.val('');
			},
			removeMention() {
				this.$mention.detach();
			},
			addNickname(nickname) {
				this.$mentionNickname.text(nickname);
			},
			addCommentNo(commentNo) {
				this.$mention.data('id', commentNo);
			},
			addMention() {
				this.$addTarget.prepend(this.$mention);
			},
			getCommentNo() {
				return this.$mention.data('id');
			}
		},
		$.observer
	);
	...
	window.oSwiper = new ssj.util.swiper({
		...
		on: {
			init: function() {
				setInitialData(mainWritingNo).then(() => {
					initModule();
				});
				oSpinner.hide();
			},
